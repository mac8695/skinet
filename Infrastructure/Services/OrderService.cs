...
namespace Infrastructure.Services
{
    public class OrderService : IOrderService
    {
        ...
        public OrderService(IUnitOfWork unotOfWork, IBasketRepository basketRepo, IPaymentService paymentService)
        {
        ...
        public async Task<Order> CreateOrderAsync(string buyerEmail, int deliveryMethodId, string basketId, Address shippingAddres)
        {
            ...
            var existingOrder = await _unotOfWork.Repository<Order>().GetEntityWithSpec(spec);

            if (existingOrder != null)
            {
                _unotOfWork.Repository<Order>().Delete(existingOrder);
